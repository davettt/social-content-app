...
const PLATFORM_INFO: Record<Platform, { name: string; icon: string; color: string }> = {
  instagram: { name: 'Instagram', icon: 'IG', color: 'from-purple-500 to-pink-500' },
  threads: { name: 'Threads', icon: '@', color: 'from-gray-700 to-gray-900' },
  twitter: { name: 'Twitter / X', icon: 'X', color: 'from-blue-400 to-blue-600' },
  linkedin: { name: 'LinkedIn', icon: 'in', color: 'from-blue-600 to-blue-800' },
};

export function ExportPage() {
  const { projectId } = useParams<{ projectId: string }>();
  const { isLoading } = useProject(projectId);
  const { platforms, caption, hashtags, selectedMediaIds, editedImages, collages } = useComposerStore();
  ...
  const hasContent = caption || selectedMediaIds.length > 0 || collages.length > 0;
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {!hasContent ? (
        <div className="card p-12 text-center">
          ...
          <p className="text-gray-500">Create a post first, then come back to export.</p>
        </div>
      ) : (
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="card p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Selected Platforms</h2>
            ...
            {exportResult ? (
              <div className="space-y-6">
                ...
                <div className="text-center p-6 bg-green-50 rounded-xl">
                  ...
                  <p className="text-lg font-medium text-gray-900 mb-4">Ready to download!</p>
                  <a
                ...
                <div className="border-t pt-6">
                  ...
                  <ol className="space-y-3 text-sm text-gray-600">
                    <li className="flex gap-3">
                      ...
                      <span>Open <strong>Finder</strong> and go to your <strong>Downloads</strong> folder</span>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <span>Find the downloaded ZIP file and <strong>double-click</strong> to extract it</span>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <span>Select the files you want, <strong>right-click</strong> and choose <strong>Share → AirDrop</strong></span>
                    </li>
                    <li className="flex gap-3">
                  ...
                  <p className="mt-4 text-xs text-gray-500">
                    Tip: Make sure AirDrop is enabled on your iPhone (Settings → General → AirDrop → Everyone or Contacts Only)
                  </p>
                </div>
              </div>
            ) : (

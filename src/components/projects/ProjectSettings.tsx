...
export function ProjectSettings() {
  ...
  const [activeTab, setActiveTab] = useState<"business" | "brand" | "contact">(
    "business",
  );
  ...
  const [businessInfo, setBusinessInfo] =
    useState<Partial<BusinessInfo> | null>(null);
  const [brandKit, setBrandKit] = useState<Partial<BrandKit> | null>(null);
  const [contactInfo, setContactInfo] = useState<Partial<ContactInfo> | null>(
    null,
  );
  ...
  const handleAnalyzeWebsite = async () => {
    ...
    if (result.success && result.analysis) {
      ...
      const newBrandKit = {
        ...
        fonts: {
          heading:
            analysis.suggestedFonts?.heading ||
            brandKit?.fonts?.heading ||
            "Inter",
          body:
            analysis.suggestedFonts?.body || brandKit?.fonts?.body || "Inter",
        },
        colorPalette:
          analysis.extractedColorPalette || brandKit?.colorPalette || [],
      };
      ...
      const newContactInfo = {
        ...
        socialHandles: {
          ...
          instagram:
            analysis.socialHandles?.instagram ||
            contactInfo?.socialHandles?.instagram,
          twitter:
            analysis.socialHandles?.twitter ||
            contactInfo?.socialHandles?.twitter,
          linkedin:
            analysis.socialHandles?.linkedin ||
            contactInfo?.socialHandles?.linkedin,
        },
      };
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
            ...
            <div className="flex items-center justify-between pt-2">
              <p className="text-xs text-gray-500">
                AI will extract brand info, colors, and business details from
                your website
              </p>
              <Button
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <span className="text-xs text-gray-400 ml-2">
                  (click to set as primary/secondary/accent)
                </span>
              </label>
              <div className="flex flex-wrap gap-2">
                {brandKit.colorPalette.map((color, index) => (
                  <div key={index} className="relative group">
                    ...
                    <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 hidden group-hover:flex gap-1 bg-white shadow-lg rounded-lg p-1 z-10">
                      <button
                        onClick={() =>
                          setBrandKit({ ...brandKit, primaryColor: color })
                        }
                      ...
                      <button
                        onClick={() =>
                          setBrandKit({ ...brandKit, secondaryColor: color })
                        }
                        className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded"
                      ...
                      <button
                        onClick={() =>
                          setBrandKit({ ...brandKit, accentColor: color })
                        }
                        className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded"
                        title="Set as Accent"
                      >
              ...
              <p className="text-xs text-gray-400 mt-2">
                Hover over a color and click P (Primary), S (Secondary), or A
                (Accent) to assign it
              </p>
            </div>
          )}
